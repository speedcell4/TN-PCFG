...
def _kernel_bwd_merge(
    ...
    do = tl.load(out_normalized + b_idx * stride_out0 + start * stride_out1 + tl.arange(0, BLOCK_R1), mask=mask,
                 other=0)
...
def kernel_log_and_diagonal_copy(
    ...
    tl.store(alpha_c + b_idx * stride_alpha_c1 + start * stride_alpha_c2 + (start + w) * stride_alpha_c3 + tl.arange(0,
                                                                                                                     BLOCK_R1),
             out_log, mask=mask)
...
def _bwd_log_and_diagonal_copy(
    ...
    do = tl.load(
        alpha_c + b_idx * stride_alpha_c1 + (start + w) * stride_alpha_c2 + (start) * stride_alpha_c3 + tl.arange(0,
                                                                                                                  BLOCK_R1),
        mask=mask, other=0)
