    ...
    def download(self, dev_ratio=0.1, re_download=False) -> str:
        ...
        if not os.path.exists(os.path.join(data_dir, 'dev.txt')):
            if dev_ratio > 0:
                ...
                try:
                    with open(os.path.join(data_dir, 'train3.txt'), 'r', encoding='utf-8') as f, \
                        ...
                        for line in f:
                            if random.random() < dev_ratio:
                                ...
                                f1.write(line)
                    os.remove(os.path.join(data_dir, 'train3.txt'))
                    os.renames(os.path.join(data_dir, 'middle_file.txt'), os.path.join(data_dir, 'train3.txt'))
                finally:
